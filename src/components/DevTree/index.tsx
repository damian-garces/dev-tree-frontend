...
export default function DevTree({ data }: DevTreeProps) {
  ...
  const handleDragEnd = (e: DragEndEvent) => {
    ...
    if (over && over.id) {
      ...
      const orderedLinks = arrayMove(enabledLinks, prevIndex, newIndex);
      setEnabledLinks(orderedLinks);

      const disabledLinks: SocialNetwork[] = JSON.parse(data.links).filter((link: SocialNetwork) => !link.enabled);
      const links = orderedLinks.concat(disabledLinks);
        ...
        <main className="mx-auto max-w-5xl p-10 md:p-0">
          ...
          <div className="flex flex-col md:flex-row gap-10 mt-10">
            ...
            <div className="w-full md:w-96 bg-slate-800 px-5 py-10 space-y-6">
              ...
              {
                data.imageUrl && <img src={data.imageUrl} alt={data.handle} className="mx-auto max-w-[250px]" />
              }
